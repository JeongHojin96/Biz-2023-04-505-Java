...
public class BookDto {
	 ...
	 * 추상화된 데이터 요소의 type를 지정하기
	 * 페이지, 가격 항목은 정수형으로
	 * 나머지 항목은 문자열형으로 하자
	 * 
	 * 모델링된 데이터를 실제 클래스로 구현하기
	 * 각 데이터 요소의 변수 이름을 설정하기
	 * ISBN(bIsbn), 도서명(bTitle), 출판사(bPublisher), 저자(bAuthor), 발행일(bPublishDate), 페이지(bPages), 가격(bPrice)
	 * 
	 * OOP 에서 데이터 클래스를 어떤 패턴을 지정할 것인가
	 ...
	 * 각 송성의 접근을 하기위해 getter, setter method를 선언하는
	 * 패턴을 사용한다.
	 */
	 ...
	 * 
	 * 코드를 reFactoring 할때 가장 많은 문제를 발생시킬 수 있다
	 * reFactoring을 할때 가장 좋은것은 최소한의 코드 수정이다
	 * 
	 */
	...
	public BookDto(String bIsbn, String bTitle, String bAuthor, String bPublisher, String bPublishDate, int bPages,
			int bPrice) {
		super();
		this.bIsbn = bIsbn;
		this.bTitle = bTitle;
	...
	public String toString() {
		return "BookDto [bIsbn=" + bIsbn + ", bTitle=" + bTitle + ", bAuthor=" + bAuthor + ", bPublisher=" + bPublisher
