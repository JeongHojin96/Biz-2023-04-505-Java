...
public class BuyerServiceImplV1 implements BuyerService {
	...
	public List<BuyerDto> selectAll() {
		String sql = " SELECT buid, buname, butel, buaddr, bubirth, bujob " + " FROM tbl_buyer " + " ORDER BY buid ";
	...
	/*
	 * SQL 명령문 작성 주의!!! WHERE 절 명령문을 작성할때 절대 변수와 전달값을 + 연산으로 묶어서 처리하지 말것
	 * 
	 * 다음과 같은 코드를 사용할 경우 만약 id 변수에 "0001 OR 1 = 1 " 과 같은 문자열이 담겨서 전달될 경우 WHERE 이
	 * 무력화된다. SQL 문이 DELETE 와 같은 명령문이라면 TABLE 의 모든 데이터가 삭제되는 명령이 실행된다 +
	 * " WHERE buid = " + id;
	 * 
	...
	public BuyerDto findById(String id) {
		String sql = " SELECT buid, buname, butel, buaddr, bubirth, bujob " + " FROM tbl_buyer " + " WHERE buid = ? ";
		try {
			PreparedStatement pStr = dbConn.prepareStatement(sql);
			pStr.setString(1, id);
