...
public class ScoreServiceV1 {
	 ...
	 * 전달된다
	 * 
	 * main() method 에서 보낼때 socreList 의 주소가 전달되고 makeScore() 에서 값을 변경시키면 main() 의
	 * scoreList 가 같이 변경되는 성질을 갖게 된다.
	 */
	public void makeScore(List<ScoreDto> scList) {
		for (int i = 0; i < classSize; i++) {
			...
			/*
			 * 배열에서는 먼저 배열선언, 배열초기화, 각 요소에 데이터 저장 List type 의 데이터 1. 먼저 비어있는(blank) List
			 * type 을 생성 2. Dto 객체변수를 생성 3. Dto 각 변수에 값을 setting(setter) 4. List 에 Dto를 add
			 */
		...
		for (int i = 0; i < scList.size(); i++) {
			for (int j = i + 1; j < scList.size(); j++) {
				if (scList.get(i).getTotal() > scList.get(j).getTotal()) {
					scList.add(i, scList.get(j));
					scList.add(j, scList.get(i + 1));
					scList.remove(i + 1);
					scList.remove(j + 1);
				}
//				if (scList.get(i).getTotal() > scList.get(j).getTotal()) {
//					ScoreDto scDto = scList.get(i); //새로운 scDto 데이터베이스 배열 생성
